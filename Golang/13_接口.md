...
上面的代码中定义了猫和狗，然后它们都会叫, 会发现main函数中明显有重复的代码, 如果后续再加上猪、青蛙等动物的话, 代码还会一直重复下去。那能不能把它们当成“能叫的动物”来处理呢？
...
从上面的代码中可以发现, 使用值接收者实现接口之后, 不管是dog结构体还是结构体指针*dog类型的变量都可以赋值给该接口变量。因为Go语言中有对指针类型变量求值的语法糖, dog指针`fugui`内部会自动求值`*fugui`
...
例如: 狗可以叫, 也可以动。我们就分别定义Sayer接口和Mover接口, 如下： `Mover`接口
...
<img src="https://gitee.com/featherwit0918/imgstorage/raw/master/20210816155345.png" width="60%">
