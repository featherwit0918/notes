...
// 函数的参数和返回值都是可选的，例如也可以实现一个既不需要参数也没有返回值的函数
func sayHello() {
...
定义了函数之后, 我们可以通过`函数名()`的方式调用函数 例如调用上面定义的两个函数, 代码如下
...
在语句块中定义的变量，通常在if条件判断、for循环、switch语句上使用这种定义变量的方式
...
<img src="https://gitee.com/featherwit0918/imgstorage/raw/master/20210812165533.png" width="60%">
